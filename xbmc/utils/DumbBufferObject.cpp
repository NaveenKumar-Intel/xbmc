...
std::unique_ptr<CBufferObject> CDumbBufferObject::Create(int format)
{
  return std::make_unique<CDumbBufferObject>(format);
}
...
CDumbBufferObject::CDumbBufferObject(int format) : m_format(format)
{
